        ...
        <div className="grid grid-cols-2 gap-2">
          {firstRow.map((src, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
                className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-[#FFEEE6] cursor-pointer border-0 p-0 text-left focus:outline-none focus:ring-2 focus:ring-[#E07050]/40"
            >
              <img
                src={src}
                alt=""
          ...
          {secondRow.map((src, i) => {
            ...
            return (
              <motion.button
                ...
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.95 }}
                className="relative aspect-square w-full overflow-hidden rounded-xl bg-[#FFEEE6] cursor-pointer border-0 p-0 text-left focus:outline-none focus:ring-2 focus:ring-[#E07050]/40"
              >
                <img
                  src={src}
                  alt=""
                ...
                {isLastVisible && hasMore && (
                    <span className="absolute inset-0 flex items-center justify-center bg-[#1A0A00]/60 rounded-xl text-white text-sm font-semibold">
                    +{extraCount} photos
                  </span>
                )}
              </motion.button>
      ...
      {isOpen && restaurant && (
        <>
          <motion.div
            ...
            tabIndex={0}
            aria-label="Fermer"
            className="fixed inset-0 z-[60] bg-[#1A0A00]/45"
            {...BACKDROP}
            onClick={onClose}
            onKeyDown={(e) => e.key === "Escape" && onClose()}
          />
          <motion.div
            className="fixed inset-x-0 bottom-0 z-[70] max-h-[85dvh] overflow-y-auto rounded-t-3xl bg-[#FFEEE6] shadow-2xl"
            initial={DRAWER.initial}
            animate={DRAWER.animate}
            exit={DRAWER.exit}
            transition={DRAWER.transition}
          >
            <div className="sticky top-0 z-10 flex justify-center bg-[#FFEEE6] py-2">
              <div className="h-1.5 w-12 rounded-full bg-[#F5C4A0]" aria-hidden />
            </div>
            {isExpanded && (
            <div className="px-5 pb-5 pt-1 space-y-3">
              {/* 1. Header : nom en très gros et gras */}
              <h2 className="text-3xl font-bold text-[#1A0A00]">{restaurant.name}</h2>
              ...
              <a
                ...
                className="inline-flex w-fit max-w-full min-h-[36px] items-center gap-1.5 rounded-full border border-[#F5C4A0] bg-[#FFF6F0] px-2.5 py-1 text-xs text-[#1A0A00] shadow-lg transition hover:bg-[#FFEEE6] focus:outline-none focus:ring-2 focus:ring-[#E07050]/40"
              >
              ...
              {/* 4. Description */}
              <p className="text-[15px] leading-relaxed text-[#1A0A00]">
              ...
              {galleryImages.length > 0 && (
                <section className="space-y-1.5" aria-label="Galerie photos">
                  <h3 className="text-sm font-semibold uppercase tracking-wide text-[#8B4A2A]">Galerie</h3>
                  <GalleryGrid
                ...
                <div className="flex flex-wrap gap-2 pt-0">
                  {restaurant.instagram_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 rounded-xl border border-[#F5C4A0] bg-[#FFF6F0] px-3 py-2 text-sm font-semibold text-[#C2401A] transition hover:bg-[#FFEEE6]"
                    >
